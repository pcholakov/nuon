#!/bin/sh
# stack-cli bootstrap. Downloads the right binary for the host OS/arch,
...
#
# Override the download host with STACK_CLI_BASE_URL or pin a version with
# STACK_CLI_VERSION.

if [ "${STACK_CLI_DEBUG:-}" = "true" ]; then
  set -x
fi
...
BASE_URL="${STACK_CLI_BASE_URL:-https://cdn.public.nuon.co/stack-cli}"
...
# STACK_CLI_INSTALL_DIR if you want it somewhere else.
INSTALL_DIR="${STACK_CLI_INSTALL_DIR:-$HOME/.local/bin}"
mkdir -p "$INSTALL_DIR"
...
VERSION="${STACK_CLI_VERSION:-}"
if [ -z "$VERSION" ]; then
  VERSION=$(curl -fsSL "$BASE_URL/latest.txt")
fi

echo "stack-cli: downloading ${OS}/${ARCH} (${VERSION})..." >&2
...
if [ "$success" -eq 0 ]; then
  ...
  if ! curl -fsSL -o "$DIR/$NAME" "$raw_url"; then
    echo "stack-cli: failed to download from $raw_url" >&2
    exit 1
  fi
fi

# Verify SHA256 checksum against the published checksums.txt. Skip with
# STACK_CLI_SKIP_CHECKSUM=true.
if [ "${STACK_CLI_SKIP_CHECKSUM:-}" != "true" ]; then
  sums_url="$BASE_URL/$VERSION/checksums.txt"
  if ! curl -fsSL -o "$DIR/checksums.txt" "$sums_url"; then
    echo "stack-cli: failed to fetch $sums_url" >&2
    exit 1
  fi
  expected=$(awk -v f="${NAME}_${OS}_${ARCH}" '$2 == f {print $1; exit}' "$DIR/checksums.txt")
  if [ -z "$expected" ]; then
    echo "stack-cli: no checksum entry for ${NAME}_${OS}_${ARCH} in checksums.txt" >&2
    exit 1
  fi
  if command -v shasum >/dev/null 2>&1; then
    ...
    echo "stack-cli: no shasum/sha256sum available; set STACK_CLI_SKIP_CHECKSUM=true to bypass" >&2
    exit 1
  fi
  if [ "$expected" != "$actual" ]; then
    echo "stack-cli: checksum mismatch (expected $expected, got $actual)" >&2
    exit 1
  fi
fi
...
# `stack-cli` without re-downloading.
mv "$DIR/$NAME" "$INSTALL_DIR/$NAME"
INSTALLED="$INSTALL_DIR/$NAME"

echo "stack-cli: installed to $INSTALLED" >&2
case ":$PATH:" in
  *":$INSTALL_DIR:"*) ;;
  *) echo "stack-cli: add $INSTALL_DIR to your PATH to invoke it directly." >&2 ;;
esac
